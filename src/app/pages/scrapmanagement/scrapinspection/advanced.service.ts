...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.slNo.toLowerCase().includes(term.toLowerCase())
        || tables.date.toLowerCase().includes(term)
        || tables.alloyName.toLowerCase().includes(term)
        || tables.source.toLowerCase().includes(term)
        || tables.weight.toLowerCase().includes(term);
}
